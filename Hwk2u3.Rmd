...
file_path <- "C:\\Dokumente\\Projekte\\Studium\\Programming\\R_documents\\Homework\\Second\\student-mat.csv"
schoold <- read.table(file_path, sep = ";", header = TRUE, stringsAsFactors = FALSE)
...
```
Even though the commonly used Shapiro-Wilk-test concluded our data sample to be not normally distributed, we were aware of its high sensitivity to outliers in large sample sizes and used a qqplot and an overlayed histogram to show that it is at least approximately normally distributed, which is enough of an requirement to be able to run a proper t-test.
